...
try:
    ...
    max_results = st.sidebar.slider(
        "Max results", 5, MAX_RESULTS_CAP, DEFAULT_MAX_RESULTS, 5
    )
    ...
    if fetch_btn:
        ...
        st.info(
            f"Loaded {len(df_trials)} trials in {fetch_s:.2f}s. Running BioMed NER..."
        )
        ...
        with st.spinner("BioMed NER in progress..."):
            ...
            try:
                ...
                st.session_state["timing"].update(
                    {"ner_s": float(ner_s), "n_entities": int(len(entities_df))}
                )
                st.success(
                    f"NER complete in {ner_s:.2f}s ({len(entities_df):,} mentions)."
                )
    ...
    def _heatmap_chart(
        df_long: pd.DataFrame, *, x_title: str, y_title: str, scheme: str
    ) -> alt.Chart:
        base = (
            alt.Chart(df_long)
            .mark_rect()
            .encode(
                ...
                color=alt.Color(
                    "n_trials:Q", title="Trials", scale=alt.Scale(scheme=scheme)
                ),
                tooltip=[
    ...
    with tab_studies:
        ...
        df_disp["_display"] = (
            df_disp["nct_id"].astype(str)
            + " — "
            + df_disp["title"].astype(str).str.slice(0, 90)
        )
        ...
        if entities_df is None or entities_df.empty:
            ...
            annotated_html = annotate_text_html(
                text,
                doc_ents,
                show_tag=SHOW_ENTITY_TAGS,
                max_entities=MAX_ANNOTATED_ENTITIES,
            )
    ...
    with tab_heatmaps:
        ...
        if entities_df is None or entities_df.empty:
            ...
            if dd.empty:
                ...
                st.altair_chart(
                    _heatmap_chart(dd, x_title="Drug", y_title="Disease", scheme="viridis"),
            ...
            if dg.empty:
                ...
                st.altair_chart(
                    _heatmap_chart(
                        dg, x_title="Gene/Protein", y_title="Disease", scheme="magma"
                    ),
                    use_container_width=True,
                )
        ...
        if entities_df is None or entities_df.empty:
            ...
            with right:
                ...
                if dist.empty:
                    ...
                    st.altair_chart(
                        donut.configure_view(stroke=None), use_container_width=True
                    )

                st.markdown("**Entities**")
                table = per_trial_entity_table(entities_df, nct_id=nct_id)
                st.dataframe(
                    table, use_container_width=True, height=380, hide_index=True
                )
        ...
        if entities_df is None or entities_df.empty:
            ...
            bar = (
                ...
                .encode(
                    ...
                    tooltip=[
                        alt.Tooltip("label_group:N", title="Type"),
            ...
            st.altair_chart(
                bar.configure_view(stroke=None).configure_axis(grid=False),
                use_container_width=True,
            )
            ...
            drop_cols = [
                c for c in ["label_raw", "score", "text_hash"] if c in df_disp.columns
            ]
            if drop_cols:
            ...
            df_disp = df_disp.rename(
                columns={k: v for k, v in col_map.items() if k in df_disp.columns}
            )
            ...
            ordered = [c for c in preferred if c in df_disp.columns] + [
                c for c in df_disp.columns if c not in preferred
            ]

            st.dataframe(
                df_disp[ordered].head(2000),
                use_container_width=True,
                hide_index=True,
            )
    ...
    with tab_export:
        ...
        st.download_button(
            "Download trials (CSV)", df_trials.to_csv(index=False), file_name="trials.csv"
        )
